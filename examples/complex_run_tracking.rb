...
class SimpleConversation
  ...
  def chat(user_input)
    ...
    trace(name: "analyze_input", run_type: "tool", inputs: { user_input: user_input }) do |run|
      @logger.info("Analyzing user input: #{user_input}")
    ...
    followup = trace(name: "generate_followup", run_type: "chain", inputs: { initial_response: first_response[:response] }) do |run|
      @logger.info("Generating followup")
...
def demonstrate_run_manager
  logger.info("===== Run Manager Operations =====")
  ...
  recent_runs.each do |run|
    logger.info("  - #{run['id']}: #{run['name']} (#{run['run_type']})")
  end
  ...
  if recent_runs.any?
    ...
    if feedback.any?
      logger.info("Feedback for this run:")
      feedback.each do |fb|
        logger.info("  - #{fb['key']}: #{fb['value']} (#{fb['comment']})")
      end
    else
      logger.info("No feedback for this run yet")
      ...
      logger.info("Added feedback to the run")
    end
    ...
    if run['child_run_ids']&.any?
      ...
      logger.info("Run tree successfully retrieved")
    end
  end
end

# Main execution
logger.info("Starting complex run tracking example")
...
Langsmith.trace(name: "conversation_flow", run_type: "chain", tags: ["example"]) do |parent_run|
  ...
  logger.info("Initial response: #{result[:initial_response]}")
  logger.info("Follow-up: #{result[:followup]}")
...
logger.info("Complex example completed!")
